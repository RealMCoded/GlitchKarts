...
console.log("Fork of Unreal Engine 4.27.2 with HTML5 ES3 (WebGL 2) platform - see https://github.com/SpeculativeCoder/UnrealEngine-HTML5-ES3 for more information.");

console.log("Emscripten version: 4.0.12");
...
var Module = {
	...
	UE4_indexedDBName: 'UE4_assetDatabase_NMKART', // this should be an ascii ID string without special characters that is unique to the project that is being packaged
	UE4_indexedDBVersion: 202511140308, // Bump this number to invalidate existing IDB storages in browsers.
};
...
Module.arguments = sessionStorageCommandLine != null ? [sessionStorageCommandLine] : ['../../../NMKART/NMKART.uproject','-stdout',];
...
Module.locateFile = function(name) {
    // --- YOUR CUSTOMIZATION ---
    // Set this to the base URL of your R2 bucket where the game assets are stored.
    // Make sure this URL ends with a slash!
    var assetBaseUrl = "https://cdn.glitchkarts.com/";
    // --- END CUSTOMIZATION ---
...
//	if (!dataFileIsGzipCompressed && name.split('.').slice(-1)[0] == 'data') serveGzipped = false;
    ...
    // Determine if we need the .gz extension
	var finalName = serveGzipped ? (name + '.gz') : name;

    // Prepend your bucket URL to the final asset name
	return "/data/" + name;
};
...
//	}
	...
	function withIndexedDB(db) {
		...
		var mainCompiledCode = fetchFromIndexedDB(db, 'wasmModule').then(function(wasmModule) {
			...
			return download(Module.locateFile('NMKART-HTML5-Shipping.wasm'), 'arraybuffer').then(function(wasmBytes) {
		...
		// ----------------------------------------
		// MAIN JS
		var mainJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('NMKART-HTML5-Shipping.js'), 'blob').then(function(data) {
		...
		// ----------------------------------------
		// MORE JS
		var dataJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('NMKART-HTML5-Shipping'+dataVariant+'.data.js'));
...
/* // The following code would download and store the .data file as a Blob, which should be more efficient than loading an ArrayBuffer. However that seems to be buggy, so avoid it for now.
			fetchOrDownloadAndStore(db, Module.locateFile('NMKART-HTML5-Shipping.data')).then(function(dataBlob) {
				return readBlobToArrayBuffer(dataBlob).then(function(dataArrayBuffer) {
					Module['preloadedPackages'] = {};
					Module['preloadedPackages'][Module.locateFile('NMKART-HTML5-Shipping.data')] = dataArrayBuffer;
...
// Instead as a fallback, download as ArrayBuffer. (TODO: Figure out the bugs with the above, and switch to using that one instead)
			fetchOrDownloadAndStore(db, Module.locateFile('NMKART-HTML5-Shipping'+dataVariant+'.data'), 'arraybuffer').then(function(dataArrayBuffer) {
				Module['preloadedPackages'] = {};
				Module['preloadedPackages'][Module.locateFile('NMKART-HTML5-Shipping'+dataVariant+'.data')] = dataArrayBuffer;
				return dataJsDownload.then(addScriptToDom);
			});
